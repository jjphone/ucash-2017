...
// update #repay-values
  function updateRepayValues( amount ){
    ...
    values.eq(0).text( numWithCommas( amount * 0.13, '' )  );
    values.eq(1).text( numWithCommas( amount * 0.2, '' )  );
    values.eq(2).text( numWithCommas( amount * 0.27, '' )  );
    values.eq(3).text( numWithCommas( amount * 0.34, '' )  );
  }

// convert number to currency with thousand separator
  function numWithCommas(x, prefix){
    if ( prefix == null ) 
      prefix = '';
    x = Math.round(x);
